...
getPackageAvailableVersionsR (PathPackageName pkgName) =
  cacheJSON $ do
    ...
    let toPair v =
          [toJSON $ showVersion v, toJSON $ renderUrl $ alternateVersionUrl v]
    return $ toJSON $ map toPair $ sort vs
  where
    alternateVersionUrl v =
      PackageVersionR (PathPackageName pkgName) (PathVersion v)
  ...
  findPackageWithLatest pkgName version $ \pkg@D.Package {..} latestPkg -> do
    ...
    content <-
      defaultLayout $ do
        ...
        let isLatest = version == D.pkgVersion latestPkg
...
getPackageVersionModuleDocsR ::
     PathPackageName -> PathVersion -> Text -> Handler Html
...
findPackage :: PackageName -> Version -> (D.VerifiedPackage -> Handler r) -> Handler r
...
parseUploadedPackage ::
     Value -> Handler (Either (ABE.ParseError D.PackageError) D.UploadedPackage)
...
displayJsonError value e =
  case e of
    ABE.InvalidJSON _ -> ["The file you submitted was not valid JSON."]
    ABE.BadSchema _ _ -> ABE.displayError D.displayPackageError e ++ extraInfo
  ...
  where
    extractVersion =
      ...
      toString >=> (D.parseVersion' . unpack)
    toObject json =
      case json of
  ...
  -- Some extra information about what might have caused an error.
    extraInfo =
      case extractVersion value of
        Just v
          | v > P.version ->
            let pursuitVersion = pack (showVersion P.version)
             in [ "Usually, this occurs because the JSON data was generated with a newer " <>
                ...
                , "This data was generated with " <>
                  pack (showVersion v) <> " of the compiler."
